...
export const grepCommand = async function* (stdin, args, { parseArgs, std: { out, err }, isStdoutToConsole, color }) {
            ...
            if (pattern === "")
                ...
                if (matchIndexes.length !== 0) {
                    ...
                    for (let i = 0; i < matchIndexes.length; i++) {
                        ...
                        const lastSegmentEnd = i === 0 ? 0 : matchIndexes[i - 1] + pattern.length;
                        result += line.substring(lastSegmentEnd, matchIndex);
                        const found = line.substring(matchIndex, matchIndex + pattern.length);
                        result += isColored ? color[matchColor](found) : found;
                    }
                    result += line.substring(matchIndexes[matchIndexes.length - 1] + pattern.length);
                    yield out(result);
                }
