import { applyStyleCommand, classesMatch, getStyleClasses, parseText, } from "./utils";
export default class HtmlTerminal {
    constructor(parent) {
        this.isOpen = false;
        this.buffer = "";
        this.listeners = {};
        ...
        parent.addEventListener("keydown", (e) => {
            e.preventDefault();
            for (let lisneter of Object.values(this.listeners))
                lisneter(e); // todo а не сломается ли
        });
        this.width = 400;
        this.linPos = 0;
    }
    ...
    closed() {
        if (!this.closedTermialPromise)
            throw new Error("Terminal not binded");
        ...
        for (let span of this.parent.childNodes) {
            ...
            if (spanStartIndex + text.length >= this.linPos) {
                ...
                if (span.nodeType === Node.TEXT_NODE) {
                    ...
                    span.textContent = "";
                    if (preText !== "")
                        span.appendChild(document.createTextNode(text.substring(0, this.linPos - spanStartIndex)));
                    span.appendChild(this.cursor);
                    if (nextText !== "")
                        span.appendChild(document.createTextNode(text.substring(this.linPos - spanStartIndex)));
                }
                return;
            }
